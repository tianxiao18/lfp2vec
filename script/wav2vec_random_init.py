...
def split_trials(trial_length, fixed_test_count=12, val_size=0.25, random_state=42):
    ...
    if trial_length >= fixed_test_count + 4:
...
def run_wave2vec2(sessions, sess, sampling_rate, acronyms_arr):
    ...
    custom_features = Features({
        'label': ClassLabel(num_classes=5, names=acronyms_arr),
    ...
    # X_train_upsampled, X_val_upsampled, X_test_upsampled = X_train, X_val, X_test
    ...
    if load_data and os.path.exists(f"{data_loading_path}/{session}_dataset_dict.pickle"):
        print(f"Loading from {data_loading_path}/{session}_dataset_dict.pickle")
        with open(f"{data_loading_path}/{session}_dataset_dict.pickle", 'rb') as f:
    ...
    w2v2_config = {"vocab_size": 32, "hidden_size": 768, "num_hidden_layers": 12, "num_attention_heads": 12,
                ...
                "layer_norm_eps": 1e-5, "feat_extract_norm": "group", "feat_proj_dropout": 0.0,
    ...
    train_config = {'epoch': 50, 'lr': 1e-5}

    ri_tag = 'rand_init' if rand_init else 'pretrained'
    ssl_tag = 'ssl' if ssl else 'nossl'
    wandb.init(project="wav2vec_ssl", config=w2v2_config, name=f"{session}-{ri_tag}-{ssl_tag}-exp", reinit=True)
    ...
    print(X_train_upsampled.shape, X_val_upsampled.shape, X_test_upsampled.shape)
    ...
    ssl_model = Wav2Vec2ForPreTraining.from_pretrained(f"{output_path}/{session}/ssl_model/")
    # w2v2_config = {"vocab_size": 32, "hidden_size": 768, "num_hidden_layers": 12, "num_attention_heads": 12,
    #                "intermediate_size": 3072, "hidden_act": "gelu", "hidden_dropout": 0.1,
    #                "attention_dropout": 0.1, "final_dropout": 0.1, "initializer_range": 0.02,
    ...
    file_path = os.path.join(output_path, f'{session}_{ri_tag}_{ssl_tag}_{random_string}_results.pickle')
    file_obj = {
    ...
    if data != "All":
        for i in range(len(sessions_list)):
            if sessions_list[i] in to_skip:
                ...
                continue
            run_wave2vec2(sessions_list, sessions_list[i], int(sampling_rate), hc_acronyms)
    else:
