...
def run_wave2vec2(sessions, sess, sampling_rate):
    label_json = {"0": "CA1", "1": "CA2", "2": "CA3", "3": "DG", "4": "VIS"}
    ...
    test_features, test_labels, test_trials, test_chans = load_preprocessed_data(pickle_path, file_path, data_type)
    features, labels, trials, chans = [], [], [], []
    for sessions in file_path:
        ...
        temp_test_chans = [f"{sessions}_{str(temp_trials[i])}_{str(temp_test_chans[i])}" for i in range(len(temp_trials))]
    ...
    print(f"Train: {train_tr_idx}")
    print(f"Val: {val_tr_idx}")
    print(f"Test: {test_tr_idx}")
    ...
    all_features, all_labels, all_trials, all_chans = load_preprocessed_data(pickle_path, session_list, data_type)
    ...
    if os.path.exists(os.path.join(output_path, f'{session}_results.pickle')) and 1 == 0:
