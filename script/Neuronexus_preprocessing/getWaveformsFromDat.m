...
% high level summary: for each single unit
% 1. extract raw waveforms for all channels centered at spike time of this unit
% 2. filter the waveforms
% 3. for each channel, computes the average waveform across all spike times
% 4. (original): select the channel with max waveform amplitude, store the channel and its waveform
% 4. (new): store all channels and their averaged waveform for this unit
...
%     end
    ...
    avgWaveform_allChannels = mean(wfF(goodChannels,:,:), 3);
    spikes.avgWaveform{ii} = avgWaveform_allChannels;
