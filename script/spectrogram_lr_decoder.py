...
def run_pipeline(sessions, session_config, sweep_config):
    ...
    for i in range(len(sessions)):
        ...
        run = wandb.init(project="spectrogram_lr_decoder", name=f"{sessions[i]}_{group}_decoder_run",group=group, config=sweep_config, reinit=True)
        ...
        model = ContrastiveLearningWithLR(encoder=None, input_dim=input_size, output_dim=5).to(device)
        ...
        file_path = os.path.join(session_config['pickle_path'], f'{session_names[i]}_lr_results.pkl')
        ...
        print(f"target session {sessions[i]} test accuracy: ", decoder_test_metrics["accuracy"],
              ...
              f"target session {sessions[i]} test f1 score: ", decoder_test_metrics["weighted_f1"])
        
        wandb.log(decoder_test_metrics)
        accuracy_scores.append(decoder_test_metrics["accuracy"])
        f1_scores.append(decoder_test_metrics['weighted_f1'])
...
if __name__ == "__main__":
    ...
    results_path = '/scratch/th3129/region_decoding/results/spectrogram_lr_decoder.csv'
