...
def arg_parser():
    ...
    parser.add_argument('--data', type=str, help='Dataset to use: Allen or ibl', default='All')
...
output_path = f"../results/custom_mixes/wave2vec2/across_session"
...
def run_wave2vec2():
    session_list = ['719161530', '794812542', '778998620', '798911424', '771160300', '768515987']
    sess = ['AD_HF01_1', 'AD_HF02_4', 'AD_HF03_1', 'AD_HF03_2', 'NN_syn_01', 'NN_syn_02']  # Test sessions
    pickle_path = "spectrogram/Allen"  # Path to the preprocessed data
    pretrained_model = ""
    session = "Train_Allen_Test_NN"
    ...
    test_features, test_labels, test_trials = load_preprocessed_data(pickle_path, file_path, data_type)
    features, labels, trials = [], [], []
    for sessions in file_path:
        ...
        temp_test_labels = [f"{sessions}_{label}" for label in temp_test_labels]
    ...
    all_features, all_labels, all_trials = load_preprocessed_data(pickle_path, session_list, data_type)
    for sess in train_session_list:
        ...
        temp_train_labels = [f"{sessions}_{label}" for label in temp_train_labels]
    ...
    for sess in val_session_list:
        ...
        temp_val_labels = [f"{sessions}_{label}" for label in temp_val_labels]
    ...
    if pretrained_model == "":
        ...
        model = AutoModelForAudioClassification.from_pretrained(
            pretrained_model, num_labels=num_labels, label2id=label2id, id2label=id2label
        )
    ...
    file_obj = {
        ...
        'train_chance_accuracy': train_chance_accuracy,
        'val_label_counts': val_label_counts,
        'val_chance_accuracy': val_chance_accuracy,
        'test_label_counts': test_label_counts,
        'test_chance_accuracy': test_chance_accuracy,
