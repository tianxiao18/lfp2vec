...
def run_pipeline(sessions, session_config, sweep_config):
    ...
    for i in range(len(sessions)):
        ...
        run = wandb.init(project="simclr_lr_decoder", name=f"{sessions[i]}_{group}_decoder_run",group=group, config=sweep_config, reinit=True)
        ...
        model = ContrastiveLearningWithLR(encoder, input_dim=sweep_config['latent_size'], output_dim=5).to(device)
        ...
        file_path = os.path.join(session_config['pickle_path'], f'{session_names[i]}_simclr_results.pkl')
...
if __name__ == "__main__":
    ...
    # within_accs, across_accs = load_results('/scratch/th3129/region_decoding/results/mlp_decoder_3s_summary.csv')
