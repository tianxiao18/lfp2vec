...
def run_pipeline(sessions, session_config, sweep_config):
    ...
    for i in range(len(sessions)):
        ...
        run = wandb.init(project="ResNet_simclr", name=f"{sessions[i]}_{group}_random_init_run",group=group, config=sweep_config, reinit=True)
        ...
        encoder = ResNetEncoder(input_size=sweep_config['latent_size'], output_size=sweep_config['latent_size'], use_projector=True, pretrained=session_config['pretrained']).to(device)
...
def run_pipeline_wrapper():
    ...
    session_config = {
        ...
        'session_idx': False,
        'augmentation_type': "temporal_masking",
        'pretrained': False
    }
...
if __name__ == "__main__":
    ...
    #     'encoder_epochs': {'values': [10, 20, 50]},
    ...
    # sweep_id = wandb.sweep(sweep_config, project="resnet_contrastive_sweep")
    ...
    #     'encoder_learning_rate': 4.7e-5,
    #     'decoder_learning_rate': 3.8e-5,
    #     'batch_size': 40,
    #     'temperature': 0.2828
    # }
    ...
    wandb_config = {
        ...
        'latent_size': 512,
        'spectrogram_size': 500,
        'encoder_learning_rate': 5.5e-5,
        'decoder_learning_rate': 7.3e-5,
        'batch_size': 32,
        'temperature': 0.7134
    }
